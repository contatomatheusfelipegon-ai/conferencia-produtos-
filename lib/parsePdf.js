...
function limparDescricao(desc) {
  // Remove duplicações de continuação: "KG KG" → "KG", "TABL. TABL." → "TABL."
  // "M 1,8 KG M 1,8 KG" → "M 1,8 KG"
  desc = desc.replace(/\b(.{2,20})\s+\1\b/g, '$1');
...
function parseResto(resto) {
  // Insere espaço antes de lote grudado na descrição
  // Ex: "PURO WHEY BAUN - 909 GR2628147 05/28" → "PURO WHEY BAUN - 909 GR 2628147 05/28"
  // Ex: "HMB - 120 TABL.2628097 04/28"         → "HMB - 120 TABL. 2628097 04/28"
  // Insere espaço entre descrição e lote grudado:
  // Caso 1: letra seguida de 6+ dígitos: "GR2628138" → "GR 2628138"
  resto = resto.replace(/([A-Za-z])(\d{6,})/g, '$1 $2');
  // Caso 2: dígito grudado antes do lote (ex: "1,82527080" → "1,8 2527080")
  // Lotes TOTVS: 6-8 chars, começam com 2 (ano 26xx, 27xx, 28xx, 29xx)
  // Aplica quando: dígito + bloco começando com 2 + 5-7 chars + espaço + MM/AA
  resto = resto.replace(/(\d)(2\d{5,7}[A-Z]?)(?=\s+\d{2}\/\d{2})/g, '$1 $2');
