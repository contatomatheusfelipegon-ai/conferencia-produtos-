 ...
 *
 * Suporta dois formatos de código de produto:
 *   - Formato Performance (PN):  PN8092, PN8704 ...
 *   - Formato GNT:               GNT8762, GNT8790 ...
 *
 ...
 *
 *   FORMA A (GNT) — descrição curta:
 *     x=16:  " 150  GNT8762 HMB 120 TABL. - GNT"
 *     x=351: "2628097    04/28  37,97 5695,50"
 *
...
// Aceita códigos PN (Performance), GNT e qualquer prefixo de 2-4 letras seguido de dígitos
...
const RE_IGNORA   = /^[-_]{5,}|^Qtde\s|^Folha|^SIGA\s|^Hora|^Data\s|^Cliente\s|^Endere|^Tipo\s|^CNPJ|^Repres|^Volume\s|^Nat\.|^Frete|^\d+o\s+Vencto|^Transp\.|^Ender\.|^NUM\.CXA|^SEPARADO|^EMBALADO|^CONFERIDO|^FINANCEIRO|^\d{3,}\s+[\d.,]|^Pedido de Venda|^Produto\/Descricao|^Peso\s/;
...
function extrairCabecalho(linhas) {
  ...
  // Número do pedido: aceita "Pedido de Venda - 256660 /" (GNT) ou "Pedido de Venda - 256660" (PN)
  const pedido =
    m('Pedido de Venda\\s*[-–]\\s*(\\d+)') ||
    m('Pedido\\s*n[oº°]?\\.?\\s*(\\d+)');

  // Cliente: aceita "(04977 /01)" (GNT com espaço) ou "(M04977/01)" (PN)
  const cliente =
    m('Cliente\\s*:\\s*(.+?)\\s*\\(\\d') ||
    m('Cliente\\s*:\\s*(.+?)\\s*\\(M\\d');
...
function extrairProdutos(linhas) {
  ...
  while (i < linhas.length) {
    ...
    const { lote, validade, sufixoEsq } = extrairLoteValidade(restoEsq, dir);
